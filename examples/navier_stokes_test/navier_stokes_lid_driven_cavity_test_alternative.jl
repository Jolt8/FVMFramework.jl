...
for i in eachindex(u_named)
    step_u = u_named[i]
    vel_vec = [SVector{3, Float64}(step_u.u_vel[c], step_u.v_vel[c], step_u.w_vel[c]) for c = 1:n_cells]
    u_named[i] = merge_properties(u_named[i], ComponentVector(velocity = vel_vec,))

    flow_vec = [SVector{3, Float64}(step_u.u_flow[c], step_u.v_flow[c], step_u.w_flow[c]) for c = 1:n_cells]
